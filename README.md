# 广州敏哥聊财税｜视频号内容生产库

本仓库以“项目 Agent → 专项 Skills → 内容知识库”组织“广州敏哥聊财税”微信视频号的内容资产与生产流程。

内容围绕账号定位、真实经营场景与可承接的财税服务展开，重点服务电商经营者及中小企业老板。

## 项目架构与使用入口

项目级规则由 [AGENTS.md](AGENTS.md) 管理；运营 Agent 位于 [.codex/agents/video-account-operator.toml](.codex/agents/video-account-operator.toml)，负责理解意图、识别阶段与调度 Skill。根据当前需求选择一个 Skill：

- 要保存同行文案、图片文字或自己的想法：录入灵感。
- 要获得可选选题：发起选题需求。
- 已确认一个选题，要撰写视频文案：生成文案。
- 内容已实际发布，要留存正式记录：发布并归档。

## 仓库结构与模块路径

| 模块 | 用途 | 路径 |
| --- | --- | --- |
| 账号规则 | 账号定位、表达边界、固定选项与归档规范 | `财税视频号内容库/00-首页与维护规则/` |
| 历史内容 | 已发布内容的事实记录，用于查重与覆盖判断 | `财税视频号内容库/01-历史内容/` |
| 内容地图 | 按业务、客户、痛点和内容类型查看覆盖情况 | `财税视频号内容库/02-内容地图/` |
| 选题规划 | 灵感库、候选选题与各业务方向选题池 | `财税视频号内容库/03-选题规划/` |
| 内容复盘 | 重复选题检查、内容缺口与定位变化记录 | `财税视频号内容库/04-内容复盘/` |
| 内容素材 | 标题、案例、风险场景、政策资料等写作素材 | `财税视频号内容库/05-内容素材库/` |
| Skill 模块 | 灵感录入、选题规划、文案生成、发布归档等可复用能力 | `.codex/skills/` |

四个专项 Skill 各自只执行当前阶段，不自动进入下一阶段：

| 场景 | 流程说明 |
| --- | --- |
| 录入灵感 | [idea-intake](.codex/skills/idea-intake/SKILL.md) |
| 获取选题 | [topic-planning](.codex/skills/topic-planning/SKILL.md) 与 [历史内容库读取规则](.codex/skills/topic-planning/references/history-vault-rules.md) |
| 撰写文案 | [video-copywriting](.codex/skills/video-copywriting/SKILL.md) |
| 发布归档 | [publish-archive](.codex/skills/publish-archive/SKILL.md) 与 [历史内容归档规范](财税视频号内容库/00-首页与维护规则/历史内容归档规范.md) |

## 主流程

```mermaid
flowchart TD
    A["用户需求或内容素材"] --> B{"当前要做什么？"}

    B -->|录入灵感| C["独立灵感录入"]
    C --> D["创建一篇灵感笔记"]
    D --> E["流程结束，等待后续调用"]

    B -->|需要选题| F["读取账号定位、历史内容、内容地图、选题池与可用灵感"]
    F --> G["去重、识别内容缺口、评估服务关联"]
    G --> H["输出 5 个选题方案"]
    H --> I["用户确定选题"]

    I --> J["生成并检查文案"]
    J --> K["用户确认采用"]
    K --> L["实际发布"]
    L --> M{"用户明确要求归档？"}

    M -->|否| N["保留待归档状态"]
    M -->|是| O["写入历史内容库"]
    O --> P["更新选题池、内容地图与重复检查"]
    P --> Q["沉淀延展灵感或真实反馈"]
    Q --> E
```

## 子流程：灵感录入

灵感录入是独立流程。完成录入后，不会自动进入选题、文案或归档。

```mermaid
flowchart TD
    A["用户提供文字、图片或图文混合素材"] --> B{"素材形式"}

    B -->|文字| C["原样记录用户文字"]
    B -->|图片| D["提取图片中的可读文字"]
    B -->|图文混合| E["分别记录用户文字与图片文字"]

    D --> F{"识别是否完整？"}
    F -->|是| G["标记：文字识别完整"]
    F -->|否| H["标记：文字识别不完整，不补写推断"]

    C --> I["建立单条灵感笔记"]
    E --> I
    G --> I
    H --> I

    I --> J["存入灵感库/年份/月份/"]
    J --> K["状态：待分析"]
    K --> L["流程结束"]
```

### 灵感录入规则

- 一个灵感对应一篇独立笔记，存放在 `财税视频号内容库/03-选题规划/灵感库/`。
- 同行文案可完整录入标题与正文。
- 图片仅提取可读文字，不保存、嵌入或链接图片。
- 图片文字不完整时，保留已识别内容并标记“文字识别不完整”，不补写推断。
- 用户未要求分析时，只完成录入；后续发起选题或分析时，才会读取相应灵感。

## 子流程：选题生成

每次选题由用户的当次需求触发，统一提供 5 个互不重复的方案。

```mermaid
flowchart TD
    A["用户提出选题需求"] --> B["读取账号定位与服务范围"]
    B --> C["读取已发布历史内容"]
    C --> D["读取内容地图、选题池与可用灵感"]

    D --> E["形成候选方向"]
    E --> F["检查主题、场景、目标客户与表达角度是否重复"]
    F --> G["评估需求关联、服务承接、内容缺口、差异化与事实可核验性"]

    G --> H["筛选 5 个互不重复的选题方案"]
    H --> I["说明标题、目标客户与场景、服务关联、入选理由、核验状态"]
    I --> J["用户选择其一或要求调整"]
```

候选方向主要来自以下内容：

1. 用户本次明确提出的主题或业务需求；
2. 灵感库中可用于分析的单条灵感；
3. 已发布内容可自然延展、但尚未覆盖的经营场景；
4. 各业务方向选题池中的成熟候选；
5. 内容地图识别出的客户痛点与主题缺口。

## 子流程：发布与归档

“确认文案”不等于“已发布”。只有内容实际发布且用户明确要求归档时，才创建正式历史记录。

```mermaid
flowchart TD
    A["用户确认文案"] --> B["状态：已采用"]
    B --> C["用户实际发布内容"]
    C --> D{"是否明确要求归档或写入？"}

    D -->|否| E["不写入历史内容库"]
    D -->|是| F["读取历史内容归档规范"]
    F --> G["核对实际发布标题、正文与发布日期"]
    G --> H["检查与历史内容是否冲突或重复"]
    H --> I["创建历史内容归档笔记"]

    I --> J["更新候选选题状态"]
    J --> K["更新内容地图与重复选题检查"]
    K --> L["沉淀可延展问题、真实反馈为独立灵感"]
    L --> M["归档完成"]
```

历史内容归档格式以 [历史内容归档规范](财税视频号内容库/00-首页与维护规则/历史内容归档规范.md) 为准；已发布内容库是内容查重与覆盖判断的事实依据。

## 日常使用示例

- “录入这段同行文案到灵感库。”
- “根据当前内容库给我 5 个电商合规选题。”
- “选择第 3 个，生成视频号文案。”
- “这条已发布，请按实际标题和正文归档。”

## 内容使用边界

- 内容需服务账号定位，并落到真实经营场景。
- 政策、税率、期限、处罚、平台规则与办理流程等确定性信息，需以当前官方来源核验。
- 不虚构政策、案例、数据或处罚结果。
- 不承诺结果，不提供规避监管的表达。
